...
class PlayerController:
	...
	def update(self, gameTime):
		...
		if (keys[K_RIGHT] and self.model.x < 800 - 32):
				...
				self.model.x -= 2
		if (keys[K_UP] and self.model.x < 800 - 32):
				self.model.y -= 2
		elif (keys[K_DOWN] and self.model.x > 0):
				self.model.y += 2
...
if ( __name__ == '__main__'):
	...
	playerView = PlayerView(player, 'swordman.png')
	playerLivesView = PlayerLivesView(player, 'swordsman.png')
